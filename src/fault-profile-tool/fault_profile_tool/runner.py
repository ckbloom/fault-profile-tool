...
def run_displacement_field(fault_trace_shp: str, profile_locations_shp: str, disp_vert: str, disp_east: str,
                           disp_north: str, components: int = 3, grid_spacing: Union[float, int] = 1,
                           swath_width: Union[int, float] = 10,
                           profile_length: Union[float, int] = 100, sort_profiles: bool = False,
                           project_profiles: bool = True, save_file: str = "offsets.pkl",
                           offset_file: str = "offsets.txt", profile_left_side: str = "W",
                           gmt_directory: str = None, alternative_error: bool = False):
    assert os.path.exists(fault_trace_shp)
    assert os.path.exists(profile_locations_shp)
    assert os.path.exists(disp_vert)
    assert os.path.exists(disp_east)
    assert os.path.exists(disp_north)
    ...
    q_app = QtWidgets.QApplication(sys.argv)
    fault = ProfileFault(fault_trace, x1_tiff=disp_east, x2_tiff=disp_north, x3_tiff=disp_vert,
                         points_or_lines=plocs, length=profile_length,
                         width=swath_width, save_pickle=save_file,
                         projection_strike=37, num_components=components,
                         projection_corner=(1659854.5523033, 5327763.23573705), grid_spacing=grid_spacing, topo=True,
                         project_to_wiggly_line=not project_profiles, profile_left_side=profile_left_side,
                         gmt_directory=gmt_directory, alternative_error=alternative_error)
    aw = ApplicationWindow(fault, save_file=offset_file)
